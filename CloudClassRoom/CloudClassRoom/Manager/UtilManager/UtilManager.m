...
- (NSMutableArray *)loadCaptions:(NSString *)srtString {
    ...
    while (![scanner isAtEnd]) {
        @autoreleasepool
        {
            ...
            startString = [startarray objectAtIndex:0];
            int startInt = [self posToPosString:startString];
            ...
            NSArray *endtarray = [endString componentsSeparatedByString:@","];
            endString = [endtarray objectAtIndex:0];
            int endInt = [self posToPosString:endString];
            ...
            // END EDIT
            Caption *caption = [[Caption alloc]init];
            caption.ID = [indexString intValue];
            caption.startPos = startInt;
            caption.captionText = textString;
            caption.endPos = endInt;
            
            [arry addObject:caption];
...
- (int)posToPosString:(NSString *)posStr {
    NSArray* posStrs = [posStr componentsSeparatedByString:@":"];
    int pos = [((NSString*)[posStrs objectAtIndex:0]) intValue] * 3600 + [((NSString*)[posStrs objectAtIndex:1]) intValue] * 60 + [((NSString*)[posStrs objectAtIndex:2]) intValue] ;
    
    return pos;
}

- (NSString *)idToanswer:(NSInteger)ID {
    NSString *str = @"ABCDEFGHIJKLMNOPQRSTUVWXYZ";
    
    return [str substringWithRange:NSMakeRange(ID, 1)];
}
...
void UncaughtExceptionHandler(NSException *exception) {
    ...
    [params setObject:@"1" forKey:@"device"];
    
    PostModel *model = [[PostModel alloc] init];
    model.urlStr = [NSString stringWithFormat:device_exception, Host];
    model.params = params;
    [MANAGER_HTTP doPostJsonSync:model];
}
