...
#import "Mp3ChapterViewController.h"

@interface Mp3ChapterViewController ()

@end

@implementation Mp3ChapterViewController
...
- (void)loadDownloadCourse {
    [MANAGER_SQLITE executeQueryWithSql:sql_select_download_course_scorm_mp3 withExecuteBlock:^(NSDictionary *result) {
        ...
        if ([ims.type intValue] == 2) {
            ...
            dl.ID = [NSString stringWithFormat:@"%@_mp3",dl.ID];//谢军
            dl.imsmanifest = ims;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cpv.ID = [NSString stringWithFormat:@"%@_%@_mp3", self.courseID, ims2.identifierref];
    ...
    if ([MANAGER_UTIL isEnableWIFI]) {
        ...
        NSPredicate *thirtiesPredicate = [NSPredicate predicateWithFormat:[NSString stringWithFormat:@" ID == '%@' ", [NSString stringWithFormat:@"%@_%@_mp3", self.courseID, ims2.identifierref]]];
        NSArray *dlArray = [[DataManager sharedManager].downloadCourseList filteredArrayUsingPredicate:thirtiesPredicate];
        ...
        if (dlArray.count > 0) {
            ...
            dl.cpv = cpv;
            if ([dl.ID isEqualToString:[NSString stringWithFormat:@"%@_%@_mp3", self.courseID, ims2.identifierref]]) {
                dl.imsmanifest = ims2;
                dl.cpv = cpv;
            }
        }
...
- (void)CPVClick:(CircularProgressView *)cpv {
    ...
    clickCPV = cpv;
    NSPredicate *thirtiesPredicate = [NSPredicate predicateWithFormat:[NSString stringWithFormat:@" identifierref == '%@' ",[[cpv.ID componentsSeparatedByString:@"_"] objectAtIndex:1]]];
...
- (void)startDownloadWith:(ImsmanifestXML *)ims {
    ...
    NSString *mp3String = [clickCPV.ID stringByReplacingOccurrencesOfString:@"_mp3" withString:@""];
    [MANAGER_SQLITE executeUpdateWithSql:sql_delete_download_course(mp3String, self.typeFile)];
    [MANAGER_SQLITE executeUpdateWithSql:sql_insert_download_course(mp3String, ims.filename)];
    [MANAGER_SQLITE executeUpdateWithSql:sql_update_download(DownloadTypeCourse, ims.status, ims.progress, mp3String, self.typeFile)];
...
- (void)doDownload:(NSString *)strID {
    NSPredicate *thirtiesPredicate = [NSPredicate predicateWithFormat:[NSString stringWithFormat:@" ID == '%@_mp3' ", strID]];
    NSArray *dlArray = [[DataManager sharedManager].downloadCourseList filteredArrayUsingPredicate:thirtiesPredicate];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (alertView.tag == 10) {
        if (buttonIndex == 1) {
            NSPredicate *thirtiesPredicate = [NSPredicate predicateWithFormat:[NSString stringWithFormat:@" identifierref == '%@' ", [[clickCPV.ID componentsSeparatedByString:@"_"] objectAtIndex:1]]];
            NSArray *dlArray = [imsmanifest.cellList filteredArrayUsingPredicate:thirtiesPredicate];
...
- (void)nextClass:(BOOL)flag {
    ...
    if (flag) {
        ...
        if (selectRow == 0) {
            ...
            ImsmanifestXML *ims11 = dataArray[selectSection];
            
            if ([MANAGER_UTIL isEnableNetWork]) {
                indexPath = [NSIndexPath indexPathForRow:selectRow-1 inSection:selectSection];
            }else {
                for (NSInteger i = selectRow - 1; i >= 0; i --) {
                    ...
                    if (ims22.status == Finished) {
                        ...
                        if (i == 0) {
                            ...
                            ImsmanifestXML *ims11 = dataArray[0];
...
- (void)downloadAllCourse:(UIButton *)button {
    switch (button.tag) {
        ...
        case 51:
        {
            [[DataManager sharedManager] doLogOutWithMp3WithWareType:7];
            for (ImsmanifestXML *ims1 in dataArray) {
